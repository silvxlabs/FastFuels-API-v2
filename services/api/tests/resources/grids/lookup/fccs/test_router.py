"""
Integration tests for api/v2/resources/grids/lookup/fccs/router.py

Tests the FCCS lookup endpoint (POST /domains/{domain_id}/grids/lookup/fccs).
...
def complete_fccs_grid(firestore_client, domain_for_testing):
    """A complete LANDFIRE FCCS grid for use as a lookup source."""
    grid_data = _make_landfire_grid_data(
        domain_for_testing["id"],
        product="fccs",
        version="2022",
        band_key="fccs",
        description="LANDFIRE FCCS fuelbed codes (Fuel Characteristic Classification System)",
        name="FCCS source for lookup tests",
        status="completed",
    )
...
@pytest.fixture(scope="session")
def pending_fccs_grid(firestore_client, domain_for_testing):
    """An FCCS grid with status "pending" (not yet complete)."""
    grid_data = _make_landfire_grid_data(
        domain_for_testing["id"],
        product="fccs",
        version="2022",
...
def grid_in_different_domain(firestore_client, second_domain_for_lookup):
    """A complete FCCS grid in a different domain than domain_for_testing."""
    yield from _make_cross_domain_lookup_grid(
        ...
        band_key="fccs",
        description="LANDFIRE FCCS fuelbed codes (Fuel Characteristic Classification System)",
        name="FCCS grid in second domain",
    )


class TestCreateFccsLookup:
    """Test the POST /domains/{domain_id}/grids/lookup/fccs endpoint."""

    def route(self, domain_id):
        return f"/domains/{domain_id}/grids/lookup/fccs"

    def test_minimal_request_creates_grid(
        self, client, domain_for_testing, complete_fccs_grid
    ):
        """Minimal request with required fields creates a lookup grid."""
        request_body = {
            "source_grid_id": complete_fccs_grid["id"],
        ...
        assert data["source"]["table"] == "fccs"
        assert data["source"]["source_grid_id"] == complete_fccs_grid["id"]
        assert data["source"]["source_band"] == "fccs"

        # Check bands
        assert len(data["bands"]) == 1
        assert data["bands"][0]["key"] == "fuel_load.litter"
    ...
    def test_request_with_metadata(
        ...
        data = response.json()
        assert data["name"] == "Surface fuels from FCCS"
    ...
    def test_inherits_georeference_from_source(
        ...
        data = response.json()
        expected = complete_fccs_grid["georeference"]
    ...
    def test_source_grid_not_complete_returns_422(
        self, client, domain_for_testing, pending_fccs_grid
    ):
        """Source grid with status != 'completed' returns 422."""
        request_body = {
            "source_grid_id": pending_fccs_grid["id"],
            "bands": ["fuel_load.litter"],
        }
    ...
    def test_source_grid_missing_fccs_band_returns_422(
        self, client, domain_for_testing, grid_without_fbfm_band
    ):
        """Source grid without an 'fccs' band returns 422."""
        request_body = {
        ...
        assert response.status_code == 422
        assert "fccs" in response.json()["detail"].lower()
    ...
    @pytest.mark.parametrize(
        "example_name,example_value", ALL_FCCS_LOOKUP_EXAMPLE_VALUES
    )
