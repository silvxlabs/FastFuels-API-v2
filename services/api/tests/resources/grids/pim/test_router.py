"""
Integration tests for api/v2/resources/grids/pim/router.py

Tests the TreeMap PIM endpoint.
...
class TestCreateTreeMap:
    """Test the POST /domains/{domain_id}/grids/pim/treemap endpoint."""

    def route(self, domain_id):
        return f"/domains/{domain_id}/grids/pim/treemap"

    def test_minimal_request_creates_grid(self, client, domain_for_testing):
        """Minimal request creates a grid with default tm_id band."""
        ...
        assert data["source"]["bands"] == ["tm_id"]

        # Check single categorical band
        assert len(data["bands"]) == 1
        assert data["bands"][0]["key"] == "tm_id"
        assert data["bands"][0]["type"] == "categorical"
        assert data["bands"][0]["unit"] is None
        assert data["bands"][0]["index"] == 0

    def test_both_bands(self, client, domain_for_testing):
        """Request with both tm_id and plt_cn bands."""
        request_body = {
            "bands": ["tm_id", "plt_cn"],
        }
    ...
    def test_request_with_metadata(self, client, domain_for_testing):
        ...
        data = response.json()
        assert data["name"] == "TreeMap plot IDs"
        assert data["description"] == "Plot imputation map for inventory"
        assert data["tags"] == ["treemap", "pim"]
        assert data["source"]["version"] == "2022"
    ...
    @pytest.mark.parametrize("example_name,example_value", ALL_TREEMAP_EXAMPLE_VALUES)
    def test_documented_example_creates_grid(
        self, client, domain_for_testing, example_name, example_value
    ):
        """Each documented TreeMap example should successfully create a grid."""
