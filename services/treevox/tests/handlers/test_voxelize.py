"""Unit tests for treevox.handlers.voxelize — voxelize_inventory + its stages.
...
from treevox.handlers.voxelize import (
    DEFAULT_MAX_WORKERS,
    GridLayout,
    VoxelizationResult,
    _pick_worker_count,
...
class TestPlanGridLayout:
    def test_builds_chunk_locations_in_block_order(self):
        layout = handler._plan_grid_layout(
    ...
    def test_invalid_resolution_maps_to_processing_error(self):
        ...
        with pytest.raises(ProcessingError) as exc:
            handler._plan_grid_layout(grid, _fake_domain(), _sample_df())
...
class TestPrepareTreeChunks:
    def test_attaches_cache_key_and_chunk_columns(self):
        ...
        layout = handler._plan_grid_layout(_base_grid(), _fake_domain(), df)
        out = handler._prepare_tree_chunks(df, layout)
    ...
    def test_does_not_mutate_input(self):
        df = _sample_df(n=2)
        layout = handler._plan_grid_layout(_base_grid(), _fake_domain(), df)
        _ = handler._prepare_tree_chunks(df, layout)
...
class TestBuildVoxelizationResult:
    def test_shape_and_georeference_populated(self):
        ...
        layout = handler._plan_grid_layout(_base_grid(), _fake_domain(), df)
        result = handler._build_voxelization_result(layout, "gs://x/g1")
...
class TestBuildPayloads:
    def test_payload_per_chunk_with_expected_keys(self):
        ...
        payloads = handler._build_payloads(
            batch,
            union_ds,
            union_y,
            union_x,
...
class TestChunkRelativeSlices:
    def test_single_chunk_union_matches_chunk(self):
        ...
        rel_y, rel_x = handler._chunk_relative_slices(
            slice(990, 2010),
            slice(990, 2010),
            slice(990, 2010),
            slice(990, 2010),
    ...
    def test_chunk_y_before_union_raises(self):
        with pytest.raises(ProcessingError) as exc:
            handler._chunk_relative_slices(
                slice(-5, 100),
                slice(0, 100),
                slice(0, 200),
                slice(0, 200),
...
class TestMaterializeChunkBuffer:
    def _union(self, shape=(2, 20, 20), keys=("volume_fraction", "tree_id")):
        data_vars = {}
        for k in keys:
            dtype, fill = handler.storage.BAND_SPECS[k]
    ...
    def test_larger_slice_raises_union_shape_mismatch(self):
        ...
        with pytest.raises(ProcessingError) as exc:
            handler._materialize_chunk_buffer(
                union,
                "volume_fraction",
                rel_y=slice(0, 15),
                rel_x=slice(0, 15),
...
class TestChunkRngSeed:
    def test_same_inputs_produce_same_seed(self):
        a = handler._chunk_rng_seed(42, 3, 7)
        b = handler._chunk_rng_seed(42, 3, 7)
        assert a == b

    def test_different_row_col_produce_different_seeds(self):
        base = handler._chunk_rng_seed(42, 0, 0)
        assert base != handler._chunk_rng_seed(42, 0, 1)
        assert base != handler._chunk_rng_seed(42, 1, 0)

    def test_different_base_seed_produces_different_seed(self):
        """Same (row, col), different base seeds → different chunk seeds."""
        assert handler._chunk_rng_seed(42, 0, 0) != (handler._chunk_rng_seed(43, 0, 0))

    def test_seed_is_uint32_range(self):
        for r, c in [(0, 0), (999, 999), (-1, -1)]:
            seed = handler._chunk_rng_seed(12345, r, c)
            assert 0 <= seed < 2**32
    ...
    def test_deterministic_across_python_processes(self):
        ...
        expected = handler._chunk_rng_seed(42, 3, 7)
        script = (
            "from treevox.handlers.voxelize import _chunk_rng_seed; "
    ...
    def test_feeds_default_rng_without_error(self):
        ...
        seed = handler._chunk_rng_seed(42, 3, 7)
...
class TestResolveBaseSeed:
    def test_uses_source_seed_when_present(self):
        source = {"seed": 12345}
        assert handler._resolve_base_seed(source, "grid-abc") == 12345
    ...
    def test_accepts_large_seed_values(self):
        ...
        assert handler._resolve_base_seed(source, "grid-abc") == 999_999_999

    def test_coerces_to_int(self):
        """String seed (e.g. from a JSON-without-type-coercion source) → int."""
        source = {"seed": "42"}
        assert handler._resolve_base_seed(source, "grid-abc") == 42

    def test_missing_seed_falls_back_to_grid_id_hash(self):
        """Legacy grids pre-dating the seed field still work."""
        with patch.object(handler, "logger") as mock_logger:
            result = handler._resolve_base_seed({}, "legacy-grid-id")
        expected = handler.zlib.crc32(b"legacy-grid-id")
    ...
    def test_null_seed_also_falls_back(self):
        ...
        with patch.object(handler, "logger"):
            result = handler._resolve_base_seed({"seed": None}, "legacy-grid-id")
        assert result == handler.zlib.crc32(b"legacy-grid-id")

    def test_fallback_is_deterministic_for_same_grid_id(self):
        """Legacy grids still re-run reproducibly against their own grid_id."""
        with patch.object(handler, "logger"):
            a = handler._resolve_base_seed({}, "g1")
            b = handler._resolve_base_seed({}, "g1")
        assert a == b
...
class TestBuildPayloadsZeroTrees:
    def test_chunk_with_no_trees_gets_empty_frame_with_schema(self):
        ...
        dims = layout.dims
        chunk_y, chunk_x = handler.voxelize.chunk_slice(
            batch[0],
            dims["ny"],
            dims["nx"],
            layout.chunk_xy,
            overlap_cells=handler.voxelize.OVERLAP_CELLS,
        )
        union_ds = xr.Dataset(
            {
                "volume_fraction": (
...
class TestVoxelizeInventoryFlow:
    ...
    @patch("treevox.handlers.voxelize.storage.masked_merge")
    @patch("treevox.handlers.voxelize.storage.read_union")
    ...
    def test_happy_path_calls_expected_stages(
        ...
        with patch(
            "treevox.handlers.voxelize.multiprocessing.get_context"
        ) as mock_get_ctx:
            fake_pool = MagicMock()
...
class TestPersistentPool:
    ...
    def test_pool_instantiated_exactly_once(
        ...
        with (
            patch("treevox.handlers.voxelize.voxelize.CHUNK_SIZE_HORIZONTAL", 20),
            patch(
                "treevox.handlers.voxelize.multiprocessing.get_context"
            ) as mock_get_ctx,
        ):
...
class TestHaloMergeAcrossChunks:
    ...
    def _build_union(self, layout, batch, keys):
        ...
        union_y, union_x = handler.voxelize.batch_union_slices(
            batch,
            layout.dims["ny"],
            layout.dims["nx"],
            layout.chunk_xy,
    ...
    def _force_chunk_xy(self, monkeypatch, chunk_xy=20):
        ...
        """
        monkeypatch.setattr(handler.voxelize, "CHUNK_SIZE_HORIZONTAL", chunk_xy)
    ...
    def test_tree_at_seam_writes_into_neighbor_halo_and_merges(self, monkeypatch):
        ...
        merged = handler.storage.masked_merge(union_ds, results, union_y, union_x)
